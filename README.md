...
NIGHTMARE 是一个面向 [SillyTavern](https://github.com/SillyTavern/SillyTavern) 的浏览器侧扩展，将主题美化、变量引擎、自动总结、通知系统、云端资源加载等能力组合到同一个设置面板下，以模块化方式按需启用。
...
| 杂项 | **云载入角色卡** | 检测角色卡中的 `cloudId`，从云端拉取并安装世界书 / 正则等附加资源，支持"记住选择"机制 |
...
- **视觉 Visuals** —— 主题开关
- **提示词 Prompt** —— 变量引擎 / 查看器 / 宏
- **系统 System** —— 自动总结 / 通知 / 更新检查
- **杂项 Misc** —— 云载入角色卡等
...
变量名支持点号嵌套（如 `chars.Alice.hp`）。完整语义见插件内置的"变量引擎"模块说明。
